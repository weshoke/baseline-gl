#ifndef BASELINE_DRAW_COLOR_H
#define BASELINE_DRAW_COLOR_H
...
constexpr uint32_t Color(float r, float g, float b, float a = 1.f) {
  return detail::Convert(r) << 24 | detail::Convert(g) << 16 |
         detail::Convert(b) << 8 | detail::Convert(a);
}

uint32_t Color(Vec3 rgb, float a = 1.f);
uint32_t Color(Vec4 rgba);

constexpr uint32_t Color(float grey, float a = 1.f) {
