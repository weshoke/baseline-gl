...
public:
  ...
  uint32_t AttributeLocation(const std::string &name) {
  ...
  void RegisterProgram(const std::string &name, F1 &&load_file,
                       F2 &&file_exists) {
    const auto compile_shader = [](gl::Shader shader, const std::string &type,
                                   const std::string &name, auto &&load_file) {
    ...
    auto vertex_shader = compile_shader(gl::Shader(GL_VERTEX_SHADER), "vertex",
                                        name + ".vs", load_file);
    auto fragment_shader = compile_shader(gl::Shader(GL_FRAGMENT_SHADER),
                                          "fragment", name + ".fs", load_file);
    ...
    programs_.emplace(name,
                      std::make_shared<AnalyzedProgram>(std::move(program)));
  }
...
private:
  void BindAttributes(gl::Program &program) {
    ...
    //                       auto size = GLint{0};
    //                       GLenum type;
    //                       glGetActiveAttrib(program, i, max_name_size,
    //                       &name_size,
    //                       &size, &type, name.data());
  ...
  Context()
      : attributes_{{"pos", 0}, {"tex_coord", 1}},
        ...
        model_(Matrix4::Identity()), view_(Matrix4::Identity()),
        projection_(Matrix4::Identity()), color_(Colors::black) {}
