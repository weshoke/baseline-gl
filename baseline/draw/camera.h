...
public:
  ...
  Camera(const Vec3 &eye, const Vec3 &look_at, const Vec3 &up, float lens_angle,
      ...
      : mode_(Mode::Projective), eye_(eye), look_at_(look_at), up_(up),
        lens_angle_(lens_angle), aspect_(aspect), near_(near), far_(far),
        left_(0.f), right_(0.f), bottom_(0.f), top_(0.f) {}

  Camera(const Vec3 &eye, const Vec3 &look_at, const Vec3 &up, float left,
         float right, float bottom, float top, float near, float far)
      : mode_(Mode::Orthographic), eye_(eye), look_at_(look_at), up_(up),
        lens_angle_(0.f), aspect_(0.f), near_(near), far_(far), left_(left),
        right_(right), bottom_(bottom), top_(top) {}

  static Camera Default(float aspect = 1.f) {
    return Camera(Vec3(0.f, 0.f, -6.f), Vec3(0.f, 0.f, 0.f),
                  Vec3(0.f, 1.f, 0.f), 35.f, aspect);
  }

  static Camera Ortho(float left = -1.f, float right = 1.f, float bottom = -1.f,
                      float top = 1.f, float near = -100.f, float far = 100.f) {
    return Camera(Vec3(0.f, 0.f, 0.f), Vec3(0.f, 0.f, 1.f), Vec3(0.f, 1.f, 0.f),
                  left, right, bottom, top, near, far);
  }

  Vec3 Look() const { return (look_at() - eye()).Normalized(); }
  Vec3 Pan() const { return E3::CrossProduct(up(), Look()); }
  template <class Rot> void Spin(const Rot &rot) {
